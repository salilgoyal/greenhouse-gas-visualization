...
#from figure import add_trace, make_MPValve_dfs, create_TFarray, create_dict

"""Instructions:
(For ENA_AOSGHG_LOCAL_SOURCE uncomment/comment lines 39 and 116-121 in loop.py)
...
"""
def parametersMethodAOSGHG(df, year, month, fileWriteFormat):
    ...
    #This is the row and column for the graph that changes when you change the dropdown menu. Typically last graph (i.e. last row).
    variableRow = 5
    variableCol = 1
    ...
    #this boolean is for the ENA aircraft masking.
    #here's a link to the stack overflow page I made for this loop:
    #https://stackoverflow.com/questions/62081305/pandas-masking-a-specific-column-by-date
    if fileWriteFormat[:len("ENA_AOSGHG_LOCAL_SOURCE")] == "ENA_AOSGHG_LOCAL_SOURCE":
        dfLocal = pd.read_csv("LocalSource_Start_Stop_2016.csv")
        if year == '2017':
            dfLocal = pd.read_csv("LocalSource_Start_Stop_2017.csv")
        elif year == '2018':
            dfLocal = pd.read_csv("LocalSource_Start_Stop_2018.csv")
        elif year == '2019':
            dfLocal = pd.read_csv("LocalSource_Start_Stop_2019.csv")

        #Note: df.TIMESTAMP is the same thing as df['TIMESTAMP']
        df.TIMESTAMP = pd.to_datetime(df.TIMESTAMP, format = "%Y-%m-%d %H:%M")
        for index, row in dfLocal.iterrows():
            start = datetime.datetime.strptime(row[0], "%Y%m%d_%H%M")
            stop = datetime.datetime.strptime(row[1], "%Y%m%d_%H%M")
            masked = (df.TIMESTAMP >= start) & (df.TIMESTAMP <= stop)
            df.CO2_DRY_AVG_CORR_QC = df.CO2_DRY_AVG_CORR_QC.mask(masked, -1)

            #use this line below if you need to print the QC values to a text file--for debugging only
            #np.savetxt(r'QC.txt', df['CO2_DRY_AVG_CORR_QC'], fmt = '%d', delimiter = '\t')
    ...
    segregateVals = {'Position 1': 1, 'Position 2': 2, 'Position 3': 3, 'Position 4': 4}
